...
public class CBloomFilter extends BloomFilter {
	 ...
	 * @param maxNum
	 * @param errorRate
	 * @param bitCount 用于计算count的bit位数，一般来说，4位就完全够了
	 */
	public CBloomFilter(double maxNum, double errorRate, int bitCount) {
		this(optimalM(maxNum, errorRate), optimalK(maxNum, optimalM(maxNum, errorRate)), bitCount);
	}
	...
	protected CBloomFilter(BitSet counts, BitSet bloom, int m, int k, int bitCount) {
		super(bloom, m, k);
		this.counts = counts;
